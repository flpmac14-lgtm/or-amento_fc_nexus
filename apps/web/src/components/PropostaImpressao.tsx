  ...
  return (
    ...
    >
      ...
      <header className="mb-3 flex items-start justify-between">
        <div>
          <h1 className="text-base font-bold" style={{ color: AZUL_MACFAB }}>
            {mac || "MAC — "}
          </h1>
          <p className="text-xs font-medium" style={{ color: AZUL_MACFAB }}>
            {proposta.tituloServico || "—"}
          </p>
        </div>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/macfab-logo.png" alt="Macfab" className="h-10 w-auto shrink-0" />
      </header>

      <div className="mb-3 border border-black">
        <p className="py-1 text-center text-sm font-bold" style={{ color: AZUL_MACFAB }}>
          {identificacaoCliente.nomeCliente || "—"}
        </p>
        {proposta.contatoCliente && (
          <p className="pb-1 text-center text-[10px] text-blue-700">{proposta.contatoCliente}</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-x-6">
        ...
        <div className="flex flex-col gap-3">
          ...
          <section className="break-inside-avoid">
            ...
            <table className="w-full border-collapse border border-black text-[10px]">
              <thead>
                <tr>
                  <th className="border border-black px-1 py-1 font-bold">ITEM</th>
                  <th className="border border-black px-1 py-1 font-bold">QTD</th>
                  <th className="border border-black px-1 py-1 font-bold">DISCRIMINAÇÃO</th>
                  <th className="border border-black px-1 py-1 font-bold">R$ TOTAL</th>
                </tr>
              </thead>
              <tbody>
                {proposta.itensPreco.map((item, i) => (
                  <tr key={i} className="break-inside-avoid">
                    <td className="border border-black px-1 py-1 text-center">{item.item}</td>
                    <td className="border border-black px-1 py-1 text-center">{item.quantidade}</td>
                    <td className="border border-black px-1 py-1 whitespace-pre-line">{item.discriminacao}</td>
          ...
          <section className="break-inside-avoid">
            ...
            <table className="w-full border-collapse border border-black text-[10px]">
              <tbody>
                <tr>
                  <td className="border border-black px-1 py-1">IPI: {proposta.ipi}</td>
                  <td className="border border-black px-1 py-1">ICMS - PIS-COFINS: {proposta.icmsPisCofins}</td>
                  <td className="border border-black px-1 py-1">NCM: {proposta.ncm || "—"}</td>
                </tr>
              </tbody>
            </table>
          </section>
        ...
        {/* Coluna direita */}
        <div className="flex flex-col gap-3">
          <section className="break-inside-avoid border border-black p-2">
            <p className="mb-1 font-bold" style={{ color: AZUL_MACFAB }}>8. EXCLUSÕES / FORNECIDO PELO CLIENTE:</p>
          ...
          <section className="break-inside-avoid mt-2 text-right">
            <p>{CIDADE_EMISSAO}, {dataEmissao}</p>
            <p className="mt-3">Cordialmente,</p>
            <p className="mt-2 text-sm font-bold italic" style={{ fontFamily: "'Brush Script MT', cursive" }}>
      ...
      <footer className="mt-4 border-t border-zinc-300 pt-2 text-center text-[9px] text-zinc-600">
        <p className="mb-1 font-bold">&quot;ADVERTÊNCIA&quot;</p>
        <p>{AVISO_CONFIDENCIALIDADE}</p>
        <p className="mt-1 text-[8px]">{ENDERECO_MACFAB}</p>
      </footer>
    </div>
  );
}
