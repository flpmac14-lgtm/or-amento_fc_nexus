...
// Só produtos de pintura da planilha de compras — o mesmo cartão de itens
// comerciais tem parafuso, tubo etc. misturado, e o plano de pintura pedido
// (fundo/intermediária/acabamento/diluente) não tem nada a ver com isso.
const PADRAO_PINTURA = /^(TINTA|DILUENTE|VERNIZ|PRIMER|CATALIZADOR|ESMALTE|THINNER)/;

export default function CartaoInsumoPintura({
  posicaoNum,
  itemNum,
  setPosicaoNum,
  setItemNum,
  ...
  // usuário digita — pega só a compra mais recente de cada descrição (a
  // lista já vem ordenada por data desc do backend).
  ...
  function adicionar() {
    if (!descricao.trim() || !custoTotal) {
      setErro("Informe a descrição, a quantidade (em litros) e o preço por litro antes de adicionar.");
      return;
    }
    setErro("");
    onAdicionar({
  ...
  return (
    <div className="rounded-xl border border-cyan-500/30 bg-slate-900/60 p-4">
      ...
      <p className="mb-3 text-xs text-slate-400">
        ...
        de preço da aba &quot;Referência de preços&quot;. Quantidade em litros.
      </p>
      ...
      {custoTotal !== null && (
        <PainelResultadoCalculo
          linhas={[
            { rotulo: "Quantidade", valor: `${formatarNumero(quantidadeNum, 2)} L` },
            { rotulo: "Preço por litro", valor: formatarMoeda(precoNum) },
          ]}
